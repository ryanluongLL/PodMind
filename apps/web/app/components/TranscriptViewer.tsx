...
import { type Episode, translateWord, getProfile, saveWord } from '@/lib/api'
...
export function TranscriptViewer({
  ...
  // Fetch user profile to get their native language for translation
  const { data: profile } = useQuery({
    queryKey: ['profile'],
    queryFn: getProfile,
  })

  // Fetch translation when popup is open
  ...
  // Save word mutation — persists to vocabulary DB
  const saveMutation = useMutation({
    mutationFn: () => saveWord({
      word: popup!.word,
      translation: translationResult!.translation,
      contextSentence: popup!.context,
      episodeId: episode.id,
      timestampSeconds: segments.find(s => s.text === popup!.context)?.start ?? 0,
    }),
    onSuccess: () => {
  ...
  const handleSegmentClick = (segmentStart: number) => {
    const seekTime = segmentStart + 0.05
